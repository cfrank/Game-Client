...
public class OnDemandFetcher extends Requester implements Runnable {
	...
	public void run() {
		try {
			while (aBoolean1339) {
				...
				if (_client.loggedIn && aSocket1355 != null && anOutputStream1349 != null
	...
	public void method332(boolean flag, byte byte0) {
		...
		for (int j = 0; j < i; j++)
			if (flag || anIntArray1336[j] != 0) {
				setPriority((byte) 2, 3, anIntArray1365[j]);
				setPriority((byte) 2, 3, anIntArray1360[j]);
			}
	...
	public void init(Archive archive, Game client1) {
		...
		for (int k2 = 0; k2 < j1; k2++)
			anIntArray1366[k2] = class50_sub1_sub2_2.getUnsignedByte();

		_client = client1;
		aBoolean1339 = true;
		_client.startRunnable(this, 2);
	}
		...
		try {
			if (aSocket1355 == null) {
				...
				if (l - aLong1378 < 4000L)
					return;
				aLong1378 = l;
				aSocket1355 = _client.openSocket(43594 + Game.portOffset);
