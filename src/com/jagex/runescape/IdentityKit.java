...
public class IdentityKit {
	...
	public static void load(Archive archive) {
		Buffer buf = new Buffer(archive.get("idk.dat"));
		count = buf.getShort();
	...
	public boolean isHeadDownloaded() {
		boolean downloaded = true;
		for (int j = 0; j < 5; j++)
			if (headModelIds[j] != -1 && !Model.isCached(headModelIds[j]))
				downloaded = false;
	...
	public Model getHeadModel() {
		...
		for (int j = 0; j < 5; j++)
			if (headModelIds[j] != -1)
				subModels[count++] = Model.getModel(headModelIds[j]);
